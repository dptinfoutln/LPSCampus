...
ip = "10.21.74.229"
url_base = "http://" + ip + ":9998/LPS/LaGarde/"
...
class Scan:
    ...
    def putInBdd(self, token):
        urlPutScan = url_base + "scans"
        
        wifiListJson = []
        for wifi in self.lWifi:
            wifiListJson.append({
                "id": 0,
                "BSSID":wifi.BSSID,
                "Capabilities" : wifi.capabilities,
                "centerFreq0" : wifi.centerFreq0,
                "centerFreq1" : wifi.centerFreq1,
                "channelWidth" : wifi.channelWidth,
                "frequency" : wifi.frequency,
                "level" : wifi.level,
                "operatorFriendlyName" : wifi.operatorFriendlyName,
                "SSID" : wifi.SSID,
                "timestamp" : wifi.timestamp,
                "venueName" : wifi.venueName
                })
        
        pieceName = scanMan.pieceInverse[self.piece]
        urlGetPiece = url_base + "pieces/name/" + pieceName
        
        headers = {'Authorization': 'Bearer' + token}
        
        responsePiece = requests.get(urlGetPiece, headers=headers)
        if responsePiece.status_code == 200:
            
            scanJson = str({
                    "id": 0,
                    "infoScan": self.info,
                    "wifiList": wifiListJson
                    }).replace("'", '"')
            ...
            #Call REST API
            response = requests.put(urlPutScan + "?piece=" + str(responsePiece.json()["id"]), data=scanJson, headers=headers)
...
def update_Bdd():
    print("Voulez vous transférer des fichiers texte de scan vers la bdd ?\nEntrez O ou N :")
    update = input()
    
    if update == "O":
        print("Veuillez entrer votre adresse email ?")
        email = input()
        print("Veuillez entrer votre mot de passe ?")
        mdp = getpass()
    
        url = url_base + "connexion"
            
        
        emailMdpIn64b = (email + ":" + mdp).encode('ascii')
        emailMdpIn64b = base64.b64encode(emailMdpIn64b).decode('utf-8')
        print(emailMdpIn64b)

        headers = {'Authorization': 'Basic' + emailMdpIn64b}
        ...
        if token.status_code == 200:
            ...
            update_data()
            textFilesToBdd(token)
        else :
            print("Acces refusé")

def test_models():
    global scanMan, x_train, x_test, y_train, y_test, model
    print("Voulez vous essayer d'entrainer les modeles avec les donnees dans vos fichiers texte de scan? Entrez O ou N")
    test = input()
    if test == "O":
